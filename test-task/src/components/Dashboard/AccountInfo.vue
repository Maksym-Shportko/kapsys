<template>
  <section class="account">
    <div class="d-flex justify-content-between align-items-center p-3 m-2">
      <h1>Account info</h1>
      <button class="account__delete_btn">Delete Account</button>
    </div>
    <div class="bg-white p-4 m-4 account-information">
      <div class="alert alert-warning mb-4" role="alert">
        <span></span>
        <span class="mr-auto text-dark">
          <svg height="20" viewBox="-21 0 512 512" width="20" fill="orange" class="mx-3" xmlns="http://www.w3.org/2000/svg"><path d="m453.332031 229.332031c-8.832031 0-16-7.167969-16-16 0-61.269531-23.847656-118.847656-67.15625-162.175781-6.25-6.25-6.25-16.382812 0-22.632812s16.382813-6.25 22.636719 0c49.34375 49.363281 76.519531 115.007812 76.519531 184.808593 0 8.832031-7.167969 16-16 16zm0 0"/><path d="m16 229.332031c-8.832031 0-16-7.167969-16-16 0-69.800781 27.179688-135.445312 76.542969-184.789062 6.25-6.25 16.386719-6.25 22.636719 0s6.25 16.386719 0 22.636719c-43.328126 43.304687-67.179688 100.882812-67.179688 162.152343 0 8.832031-7.167969 16-16 16zm0 0"/><path d="m234.667969 512c-44.117188 0-80-35.882812-80-80 0-8.832031 7.167969-16 16-16s16 7.167969 16 16c0 26.476562 21.523437 48 48 48 26.472656 0 48-21.523438 48-48 0-8.832031 7.167969-16 16-16s16 7.167969 16 16c0 44.117188-35.882813 80-80 80zm0 0"/><path d="m410.667969 448h-352c-20.589844 0-37.335938-16.746094-37.335938-37.332031 0-10.925781 4.757813-21.269531 13.058594-28.375 32.445313-27.414063 50.941406-67.261719 50.941406-109.480469v-59.480469c0-82.34375 66.988281-149.332031 149.335938-149.332031 82.34375 0 149.332031 66.988281 149.332031 149.332031v59.480469c0 42.21875 18.496094 82.066406 50.730469 109.332031 8.511719 7.253907 13.269531 17.597657 13.269531 28.523438 0 20.585937-16.746094 37.332031-37.332031 37.332031zm-176-352c-64.707031 0-117.335938 52.628906-117.335938 117.332031v59.480469c0 51.644531-22.632812 100.414062-62.078125 133.757812-.746094.640626-1.921875 1.964844-1.921875 4.097657 0 2.898437 2.433594 5.332031 5.335938 5.332031h352c2.898437 0 5.332031-2.433594 5.332031-5.332031 0-2.132813-1.171875-3.457031-1.878906-4.054688-39.488282-33.386719-62.121094-82.15625-62.121094-133.800781v-59.480469c0-64.703125-52.628906-117.332031-117.332031-117.332031zm0 0"/><path d="m234.667969 96c-8.832031 0-16-7.167969-16-16v-64c0-8.832031 7.167969-16 16-16s16 7.167969 16 16v64c0 8.832031-7.167969 16-16 16zm0 0"/></svg>
          <strong>Verify youe email.</strong>We sent the verification link to
          <strong>user@email.com</strong>
        </span>
        <router-link class="account__reset-link" to="/">Reset the link</router-link>
      </div>
      <div class="mb-4">
        <strong class="mr-5">Email:</strong>
        <span class="ml-5">admin@gmail.com</span>
      </div>
      <div class="d-flex justify-content-between">
        <strong>Password:</strong>
        <div class="position-relative col-md-3">
          <span @click="currentPwd = !currentPwd" class="password-eye" id="eye"></span>
          <input
            class="account__password-field"
            :type="currentPwd ? 'text' : 'password'"
            id="password"
            placeholder="Current password"
          />
        </div>

        <div class="position-relative col-md-3">
          <span @click="newPwd = !newPwd" class="password-eye" id="eye"></span>
          <input
            class="account__password-field"
            :type="newPwd ? 'text' : 'password'"
            id="password"
            placeholder="Sent new password"
          />
        </div>
        <div class="position-relative col-md-3">
          <span @click="confirmPwd = !confirmPwd" class="password-eye" id="eye"></span>
          <input
            class="account__password-field"
            :type="confirmPwd ? 'text' : 'password'"
            id="password"
            placeholder="Confirm new password"
          />
        </div>
        <button class="account__save-password_btn">Save</button>
      </div>
    </div>
  </section>
</template>

<script>
export default {
  name: "AccountInfo",
  data: () => ({
    currentPwd: false,
    newPwd: false,
    confirmPwd: false,
  }),
};
</script>

<style lang="scss" scoped>
.account {
  &-information {
    border: 1px solid #bdb7b7;
    border-radius: 3px;
    height: 600px;
  }
  &__delete_btn {
    padding: 5px 15px;
    height: 40px;
    border: 1px solid rgb(189, 183, 183);
    border-radius: 3px;
    color: #bdb7b7;
    background: transparent;
  }
  &__password-field {
    padding: 5px 15px;
    border-radius: 3px;
    border: 1px solid #bdb7b7;
    // width: 270px;
    width: 100%;
  }
  &__save-password_btn {
    width: 90px;
    height: 37px;
    background-color: #79d4f1;
    border: none;
    border-radius: 3px;
    color: white;
    font-weight: 600;
  }
  &__reset-link {
    float: right;
    font-weight: 600;
    float: right;
    border: 1px solid #bdb7b7;
    padding: 0px 10px;
    border-radius: 3px;
    color: #757575;
    background-color: #f5f5f5;
  }
  .password-eye {
    background: url("../../assets/vision.png") no-repeat;
    display: block;
    height: 15px;
    width: 18px;
    position: absolute;
    bottom: 12px;
    right: 20px;
    cursor: pointer;
  }
}
</style>